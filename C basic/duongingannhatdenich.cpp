...
int main() {
    printf("Nhap kich thuoc ma tran (n m) va toa do diem bat dau: ");
    ...
    printf("Nhap toa do diem dich (i1 j1): ");
    scanf("%d %d", &destination.x, &destination.y);
    int shortest = shortestPath(start, destination);

    if (shortest != -1) {
        printf("Khoang cach ngan nhat tu diem bat dau den diem dich: %d buoc\n", shortest);
    } else {
        printf("Khong co duong di tu diem bat dau den diem dich.\n");
    }
